...
``` r
mod_late_block <- manyglm(comm_all_late_colonization_mv ~ block2 + treatments + AM_days_sc + AM_days_sc_squared + AM_days_sc:treatments + AM_days_sc_squared:treatments, family="negative.binomial",  data = Exp_design_all_late_colonization, composition = FALSE)

mod_late_no_block <- manyglm(comm_all_late_colonization_mv ~ treatments + AM_days_sc + AM_days_sc_squared + AM_days_sc:treatments + AM_days_sc_squared:treatments, family="negative.binomial",  data = Exp_design_all_late_colonization, composition = FALSE)
...
```
    ...
    ## Analysis of Deviance Table
    ## 
    ## mod_late_no_block: comm_all_late_colonization_mv ~ treatments + AM_days_sc + AM_days_sc_squared + AM_days_sc:treatments + AM_days_sc_squared:treatments
    ## mod_late_block: comm_all_late_colonization_mv ~ block2 + treatments + AM_days_sc + AM_days_sc_squared + AM_days_sc:treatments + AM_days_sc_squared:treatments
    ## 
    ## Multivariate test:
    ##                   Res.Df Df.diff   Dev Pr(>Dev)
    ## mod_late_no_block     45                       
    ## mod_late_block        43       2 51.99    0.135
    ## Arguments:
    ##  Test statistics calculated assuming uncorrelated response (for faster computation) 
    ##  P-value calculated using 999 iterations via PIT-trap resampling.
...
```
    ...
    ##  Resampling run 700 finished. Time elapsed: 0.29 minutes...
    ##  Resampling run 800 finished. Time elapsed: 0.33 minutes...
...
``` r
mod_late_exp_block <- manyglm(comm_all_late_experiment_mv ~ block2 + treatments + AM_days_sc + AM_days_sc:treatments, family="negative.binomial",  data = Exp_design_all_late_experiment, composition = FALSE)

mod_late_exp_no_block <- manyglm(comm_all_late_experiment_mv ~ treatments + AM_days_sc + AM_days_sc:treatments, family="negative.binomial",  data = Exp_design_all_late_experiment, composition = FALSE)
...
```
    ...
    ## Analysis of Deviance Table
    ## 
    ## mod_late_exp_no_block: comm_all_late_experiment_mv ~ treatments + AM_days_sc + AM_days_sc:treatments
    ## mod_late_exp_block: comm_all_late_experiment_mv ~ block2 + treatments + AM_days_sc + AM_days_sc:treatments
    ## 
    ## Multivariate test:
    ##                       Res.Df Df.diff  Dev Pr(>Dev)
    ## mod_late_exp_no_block     48                      
    ## mod_late_exp_block        46       2 46.7    0.177
    ## Arguments:
    ##  Test statistics calculated assuming uncorrelated response (for faster computation) 
    ##  P-value calculated using 999 iterations via PIT-trap resampling.
...
```
    ...
    ##  Resampling run 700 finished. Time elapsed: 0.17 minutes...
    ##  Resampling run 800 finished. Time elapsed: 0.19 minutes...
    ##  Resampling run 900 finished. Time elapsed: 0.22 minutes...
    ## Time elapsed: 0 hr 0 min 30 sec
...
lines(x = points_late_controle[,1], y = points_late_controle[,2], lwd = 3, lty = 1, col = "#009E73") #controle

lines(x = points_late_atrasado[,1], y = points_late_atrasado[,2], lwd = 3, lty = 1, col = "#D55E00") #atrasado

lines(x = points_late_fechado[,1], y = points_late_fechado[,2], lwd = 3, lty = 1, col = "#56B4E9") #fechado
...
legend(x = 100, y = 100, xjust = 1, yjust = 1, lty = c(1,1,1), lwd = 3, legend = c("Control", "Closed", "Late"), col = c( "#56B4E9", "#009E73","#D55E00"), bty = "n")
```

![](late_treatment_files/figure-gfm/unnamed-chunk-30-1.png)<!-- -->
...
![](late_treatment_files/figure-gfm/unnamed-chunk-31-1.png)<!-- -->
