---
title: "How does community variability change over time?"
...
Running the beta deviation analysis keeps gamma diversity within each survey fixed, total number of individuals in each sample fixed, and average number of species in each survey fixed. This is done through the custom function beta_deviation() that can be find in the functions folder. It heavily rely on the betadisper and null models of the vegan package.
...
```{r, echo=FALSE, eval=FALSE}
beta_data <- data.frame(observed_distances = beta_dev$observed_distances,
           expected_distances = beta_dev$expected_distances,
           deviation_distances = beta_dev$deviation_distances,
           sites = Exp_design_all_2$sites,
           treatments = Exp_design_all_2$treatments,
           block2 = Exp_design_all_2$block2,
           AM = Exp_design_all_2$AM)

write.csv(beta_data, "data/beta_data.csv")
```
...
Ok. The local diversities vary, but the averages are always the same. Always fixed.
