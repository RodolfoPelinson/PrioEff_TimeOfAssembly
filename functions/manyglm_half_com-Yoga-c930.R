...
run_models <- function(y){
  ...
  comm_first_half_mv <- mvabund::mvabund(comm_first_half)
  comm_second_half_mv <- mvabund::mvabund(comm_second_half)
  ...
  if(is.null(covariables) == FALSE){
    comm_pred_first_half <- data.frame(comm_pred_first_half, covariables)
    comm_pred_second_half <- data.frame(comm_pred_second_half, covariables)
    formula_first_half_null <- formula( paste( "comm_first_half_mv ~", paste(colnames(covariables), collapse = " + ") ) )
    formula_second_half_null <- formula( paste( "comm_second_half_mv ~", paste(colnames(covariables), collapse = " + ") ) )
  }else{
  ...
  mod_first_half_null <- mvabund::manyglm(formula_first_half_null,  family = "negative.binomial", data  = comm_pred_first_half)
  mod_first_half <- mvabund::manyglm(formula_first_half,  family = "negative.binomial", data  = comm_pred_first_half)
  mod_second_half_null <- mvabund::manyglm(formula_second_half_null,  family = "negative.binomial", data  = comm_pred_second_half)
  mod_second_half <- mvabund::manyglm(formula_second_half,  family = "negative.binomial", data  = comm_pred_second_half)
  
  R2_first_half <- R2_manyglm(mod_first_half, mod_first_half_null)
  anova_first_falf <- mvabund::anova.manyglm(mod_first_half_null, mod_first_half, resamp = "montecarlo", show.time = "all", nBoot = 999)
  
  R2_second_half <- R2_manyglm(mod_second_half, mod_second_half_null)
  anova_second_falf <- mvabund::anova.manyglm(mod_second_half_null, mod_second_half, resamp = "montecarlo", show.time = "all", nBoot = 999)
