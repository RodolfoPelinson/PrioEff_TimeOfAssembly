...
#com <- com_test
#dist = "bray"
#group <- Treatments_test
#times = 3
#transform = NULL
#seed = NULL
#type = "centroid"
#bias.adjust = TRUE
#strata = Treatments_test

MY_beta_deviation <- function(com, dist = "bray", group,
                           strata = NULL,  times = 1000, transform = NULL, seed = NULL, type = "centroid", bias.adjust = TRUE){
  ...
  if(is.null(strata)==FALSE){
    ...
    for(i in 1:times){
      com_null <- com
      for(levels_i in 1:nlevels){
        com_frac <- com_null[strata == levels[levels_i],]
        com_frac_null <-  null_com_algorithm(com_frac)
        com_null[strata == levels[levels_i],] <- com_frac_null
      }
      nm[[i]] <- com_null
    }
  }else{
    nm <- list(com)
    nm <- rep(nm, times)
    nm <- lapply(nm, FUN = null_com_algorithm)
  }
  

  
  null_coms<-array(NA, dim = c(dim(as.matrix(vegdist(nm[[1]], method = dist))),times))
  ...
  for(i in 1:length(nm)){
    ...
    if(is.null(transform)==F){
      nm[[i]] <- decostand(nm[[i]], method = transform)
    }
    null_dissim <- vegdist(nm[[i]], method = dist)
  ...
  null_exp <- matrix(NA,nrow = nrow(null_coms[,,1]), ncol = ncol(null_coms[,,1]))
  ...
  null_exp_sd <- matrix(NA,nrow = nrow(null_coms[,,1]), ncol = ncol(null_coms[,,1]))
  for(i in 1:dim(null_coms)[1]){
    for(j in 1:dim(null_coms)[2]){
